...
Texture::Texture(const std::string& name, const std::string & path) : rendererID(0), name(name), filepath(path), data(0), width(0), height(0), BPP(0)
{
    ...
    if (data) 
    {
        ...
        Log_(Log::System, Log::Texture, "Loading texture: {} {}", rendererID, name);
    }
    else
    {
        Log_(Log::Warning, Log::Texture, "Failed to load texture: {}", name );
    }
...
{
    Log_(Log::Trace, Log::Texture, "Binding texture {} to slot {}", rendererID, slot);
...
bool Texture::reload()
{
    ...
    {
        Log_(Log::Error, Log::Texture, "Reload failed (file or path missing): {}", error.what());
        return false;
    }
}